...
def load_data(data_path, test_fraction):
    ...
    #kleur bananen    = np.asarray([np.array(Image.open(join(data_path, 'Banana', banaan))) for banaan in bananen_onlyfiles])
    ...
    #kleur carambolas = np.asarray([np.array(Image.open(join(data_path, 'Carambola', carambola))) for carambola in carambola_onlyfiles])
...
# OTHER variables
data_path = r'data/2x/'
IMG_DIMS = (128,160,1)        #gray scale
VERBOSE = True
NET_VERBOSE = True
model_path = "models_trained/model_2x.h5"
checkpoint_path = "checkpoints/cp_2x.ckpt"
